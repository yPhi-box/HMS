...
- Steeper temporal decay: half-life 60d → 30d so recent facts decisively
  outrank stale ones in conflict scenarios (30d old = 50% weight)
    ...
    def search(
        ...
        **kwargs
        ...
        for result in semantic_results:
            ...
            # Temporal decay (30-day half-life: 1d=97.7%, 7d=85%, 14d=72%, 30d=50%)
            indexed_at = datetime.fromisoformat(result['indexed_at'])
            days_old = max(0, (now - indexed_at).days)
            temporal_factor = math.pow(0.5, days_old / 30)
        ...
        if self.reranker and filtered:
            rerank_pool = filtered[:10]  # Balanced pool — keeps latency under 500ms
