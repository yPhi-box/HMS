...
async def startup():
    ...
    # Auto-start transcript watcher if SESSIONS_DIR is set
    sessions_dir = os.environ.get("HMS_SESSIONS_DIR", "")
    if sessions_dir and Path(sessions_dir).exists():
        from transcript_watcher import start_transcript_watcher_background
        archive_days = int(os.environ.get("HMS_TRANSCRIPT_ARCHIVE_DAYS", "7"))
        start_transcript_watcher_background(
            sessions_dir, db_path=str(db_path),
            catch_up=True, archive_days=archive_days
        )
        print(f"Transcript watcher started for: {sessions_dir}")
    ...
    try:
        ...
        search_results = [
            SearchResult(
                ...
                line_end=r['line_end'],
                text=r['text'],
                score=r.get('blended_score', r['combined_score']),
