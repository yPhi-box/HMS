...
#   --sessions PATH  Session transcripts directory (default: auto-detect)
#   --no-service     Don't install systemd service
#   --no-plugin      Don't install OpenClaw plugin
#   --auto           Non-interactive (skip all prompts, use defaults)
#   --help           Show this help
# ============================================================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --sessions)   SESSIONS_DIR="$2"; shift 2;;
...
# Auto-detect sessions directory if not provided
...
echo ""
echo -e "${BLUE}╔══════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  HMS v2.4.1 — Hybrid Memory Server       ║${NC}"
echo -e "${BLUE}║  Local embeddings · Transcript indexing   ║${NC}"
echo -e "${BLUE}║  Zero API costs · Real-time search        ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
echo ""
...
if ! command -v python3 &>/dev/null; then
    err "Python 3 not found. Install: sudo apt install python3 python3-pip python3-venv"
fi
...
# If running from cloned repo, copy from src/
if [[ -f "$SCRIPT_DIR/src/server.py" ]]; then
    cp "$SCRIPT_DIR/src/"*.py "$INSTALL_DIR/"
    cp "$SCRIPT_DIR/src/requirements.txt" "$INSTALL_DIR/"
    log "  Source files copied from repo ✓"
# If running from repo root (files at root level)
elif [[ -f "$SCRIPT_DIR/server.py" ]]; then
    cp "$SCRIPT_DIR/"*.py "$INSTALL_DIR/"
    cp "$SCRIPT_DIR/requirements.txt" "$INSTALL_DIR/"
    log "  Source files copied ✓"
# If piped from curl, download from GitHub
elif [[ ! -t 0 ]] || [[ "$SCRIPT_DIR" == "/dev" ]] || [[ "$SCRIPT_DIR" == "/proc"* ]]; then
    log "  Downloading source from GitHub..."
    REPO_URL="https://raw.githubusercontent.com/yPhi-box/HMS/main/src"
    for f in server.py database.py search.py chunker.py embedder.py entities.py \
             indexer.py watcher.py reranker.py transcript_parser.py transcript_watcher.py \
             requirements.txt; do
        curl -sSL "$REPO_URL/$f" -o "$INSTALL_DIR/$f" || err "Failed to download $f"
    done
    log "  Source downloaded ✓"
else
    err "Can't find source files. Clone the repo or pipe install.sh from curl."
fi
...
if [[ "$INSTALL_SERVICE" == "true" ]]; then
    ...
    ENV_LINES="Environment=TRANSFORMERS_VERBOSITY=error
Environment=HMS_WATCH_PATHS=$WATCH_DIR
Environment=HMS_PORT=$PORT"
    
    if [[ -n "$SESSIONS_DIR" && -d "$SESSIONS_DIR" ]]; then
        ENV_LINES="$ENV_LINES
Environment=HMS_SESSIONS_DIR=$SESSIONS_DIR
Environment=HMS_TRANSCRIPT_ARCHIVE_DAYS=7"
    fi
...
SVCEOF
    ...
    if [[ -n "$SESSIONS_DIR" && -d "$SESSIONS_DIR" ]]; then
        log "  Transcript watcher enabled for $SESSIONS_DIR ✓"
    fi
fi
...
if [[ "$INSTALL_PLUGIN" == "true" ]]; then
    ...
    if [[ -d "$SCRIPT_DIR/openclaw-plugin" ]]; then
        PLUGIN_SRC="$SCRIPT_DIR/openclaw-plugin"
    ...
    if [[ -n "$PLUGIN_SRC" ]]; then
        ...
        log ""
        log "  To activate, add to openclaw.json:"
        log "    plugins.load.paths: [\"$PLUGIN_DIR\"]"
        log "    plugins.slots.memory: \"hms-memory\""
    else
        # Download plugin from GitHub
        log "Downloading OpenClaw plugin..."
        mkdir -p "$PLUGIN_DIR"
        PLUGIN_URL="https://raw.githubusercontent.com/yPhi-box/HMS/main/openclaw-plugin"
        for f in openclaw.plugin.json index.js; do
            curl -sSL "$PLUGIN_URL/$f" -o "$PLUGIN_DIR/$f" 2>/dev/null || true
        done
        if [[ -f "$PLUGIN_DIR/openclaw.plugin.json" ]]; then
            log "  Plugin downloaded ✓"
        else
            warn "  Plugin download failed — install manually from GitHub"
        fi
    fi
fi
...
print(f\"  Indexed: {stats['total_chunks']} chunks, {stats['total_files']} files, {stats['total_entities']} entities\")
indexer.close()
" 2>&1 | grep -E "Indexed:|chunks"
    deactivate
...
if [[ -d "$WATCH_DIR" ]]; then
    ...
    if [[ ! -f "$HMS_RULES" ]]; then
        log "Installing memory search protocol..."
...
echo ""
echo -e "${GREEN}╔══════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  HMS v2.4.1 installed successfully!       ║${NC}"
echo -e "${GREEN}╚══════════════════════════════════════════╝${NC}"
echo ""
...
if [[ -n "$SESSIONS_DIR" ]]; then
    log "Transcripts: Indexing from $SESSIONS_DIR"
fi
echo ""
